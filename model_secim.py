...
X = dataset.iloc[:, [2, 3]].values #bagımsız değişkenler #veya dataset[["Age","EstimatedSalary"]].values
y = dataset.iloc[:, 4].values #bagımlı degisken #veya dataset[["Purchased"]].values
...
2. X: bagımsız degiskenler
3. y: bagımlı degiskne
4. cv : kaç katlamalı

'''
basari = cross_val_score(estimator = classifier, X=X_train_scaled, y=y_train , cv = 4)
print(basari.mean())
print(basari.std())
...
grid_search = gs.fit(X_train_scaled,y_train)
